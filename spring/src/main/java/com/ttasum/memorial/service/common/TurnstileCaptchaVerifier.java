...
//    }
    ...
    @Override
    public boolean verifyCaptcha(String token) {
        log.debug("Turnstile secret (for verification) = '{}'", secret);

        if ("1x0000000000000000000000000000000AA".equals(secret)) {
            log.debug("테스트 모드: 더미 Secret Key 사용, 무조건 통과 처리");
            return true;
        }
        ...
        try {
            ...
            if (body == null || body.get("success") == null) {
                log.debug("Turnstile 응답이 null이거나 성공 여부 없음: {}", body);
                return false;
            }
            ...
            if (!success) {
                log.debug("Turnstile 검증 실패: {}", response.getBody());
            }

            return success;

        } catch (Exception e) {
            log.debug("Turnstile 검증 중 오류 발생", e);
            return false;
        }
    }
}
